...
// using UnityEngine.Events;//ANCHOR ADD
...
public class MoveByTouch : MonoBehaviour
{
    ...
    // Cube[] cubes;//ANCHOR ADD
    // [System.Serializable]//ANCHOR ADD
    // public class CubeEvent : UnityEvent<Cube> { }//ANCHOR ADD

    void Start()
    {
        Application.targetFrameRate = -1;
        BeTouchObj = GameObject.Find("NullObject");
        // cubes = Object.FindObjectsOfType<Cube>();//ANCHOR ADD
    }
    // public CubeEvent CubeClicked;//ANCHOR ADD
    void Update()
    {
        if (Input.touchCount == 1)
        {
            ...
            RaycastHit hit;
            // // float distance = Mathf.Infinity;//ANCHOR ADD no Need
            // Cube target = null;//ANCHOR ADD
            // foreach (var c in cubes)//ANCHOR ADD
            // {//ANCHOR ADD
            //     // var col = c.GetComponent<Collider>();//ANCHOR ADD no Need
            //     if (Physics.Raycast(ray, out hit))//ANCHOR ADD
            //     // if (col.Raycast(ray, out hit, distance))//ANCHOR ADD no Need
            //     {//ANCHOR ADD
            //         // distance = hit.distance;//ANCHOR ADD no Need
            //         target = hit.collider.gameObject.GetComponent<Cube>();//ANCHOR ADD
            //     }//ANCHOR ADD
            // }//ANCHOR ADD

            // if (target)
            // {//ANCHOR ADD
            //     CubeClicked.Invoke(target);//ANCHOR ADD
            // }//ANCHOR ADD
